//
//  QuickViewAnalyticsViewController.swift
//  Musical Notes
//
//  Created by Hayato Nakamura on 2019/12/02.
...
class QuickViewAnalyticsViewController: UIViewController {
    ...
    // MARK: - FacialEmotions
    struct newFacialEmotions: Codable {
        let anger, joy, sorrow, surprise: Int
    }

    // MARK: - TextSentiments
    struct newTextSentiments: Codable {
        let anger, disgust, fear, joy: Double
        let sadness: Double
    }
    
    @IBOutlet weak var bar_chart: BarChartView!
    @IBOutlet weak var FE_bar_chart: BarChartView!
    ...
    override func viewDidLoad() {
        ...
        title_label.text = UserDefaults.standard.string(forKey: "current_title")!
        initialize_chart()
    }
    
    func initialize_chart() {

        let u = "http://3.231.6.81:5000/emotions/" + UserDefaults.standard.string(forKey: "current_id")!
               ...
               let task = URLSession.shared.dataTask(with: url) {(data, response, error) in
                ...
                for emotion_info in book_data {
                   ...
                   DispatchQueue.main.async {
                    let emotions_text = ["anger", "sadness", "joy", "fear", "disgust"]
                    let vals = [ang, sad, joy, fear, dis]
                    ...
                    let emotions_face = ["anger", "sorrow", "joy", "surprise"]
                    let vals_face = [f_ang, f_sor, f_joy, f_sur]
                    self.FE_bar_chart.backgroundColor = #colorLiteral(red: 1, green: 1, blue: 1, alpha: 1)
                    self.FE_bar_chart.animate(xAxisDuration: 2.0, yAxisDuration: 2.0)
    ...
    func setChart_face(dataPoints: [String], values: [Int]) {
        ...
        FE_bar_chart.noDataText = "You need to provide data for the chart."
        ...
        FE_bar_chart.leftAxis.enabled = false
        //FE_bar_char.rightAxis.enabled = false
        //FE_bar_char.xAxis.enabled = false
        
        FE_bar_chart.xAxis.valueFormatter = IndexAxisValueFormatter(values: dataPoints)
        FE_bar_chart.xAxis.granularityEnabled = true
        FE_bar_chart.xAxis.labelPosition = .bottom
